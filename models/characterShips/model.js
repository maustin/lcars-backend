...
function readOne(id, callback) {
	database.get('SELECT * FROM character_ship WHERE id = ?', [id], callback);
}
...
function readAllWithCharacterId(id, callback) {
	...
	database.all('SELECT character_ship.id, ships.id AS ship_id, ships.name AS "ship.name", ships.registry AS "ship.registry", effective_date FROM character_ship JOIN ships ON character_ship.ship_id = ships.id WHERE character_id = ?', [id], callback);
}

// get all with ship id
function readAllWithShipId(id, callback) {
	database.all('SELECT character_ship.id, characters.id AS character_id, characters.name AS "character.name", effective_date FROM character_ship JOIN characters ON character_ship.character_id = characters.id WHERE ship_id = ?', [id], callback);	
}

// delete
function remove(id, callback) {
	database.get('SELECT id FROM character_ship WHERE id = ?', [id], (error, data) => {
		if (data)
